...
for i = 1:size(genox.data,2)
    top(i) = max(mean(genox.onehour{i}', 'omitnan'));
end
...
% colors
    ...
    sem = (std(data{j}, 0, 2, 'omitnan')/sqrt(size(data{j},2)))';
    A(j) = shadedErrorBar(time, mean(data{j}', 'omitnan'), sem, 'lineprops', s);
...
for i = 1:genox.number_of_genos
    to_sort(i) = max(mean(data{i}', 'omitnan'));
end
